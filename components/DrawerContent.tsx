  ...
  return (
    <div className="text-white px-10 h-full bg-[#161616] rounded-l-3xl flex flex-col gap-3 justify-between items-center text-center font-xl py-10 uppercase overflow-y-scroll scrollbar-hide">
      <div>
        ...
        {/* @ts-ignore */}
        <h1 className="pt-6 text-3xl sm:text-4xl drop-shadow-lg px-10 uppercase">
          {drawerData?.name}
        </h1>
        {/* @ts-ignore */}
        <h2 className="text-md italic lowercase">
          &quot;
          {drawerData?.status != null && drawerData?.status != ""
            ? drawerData.status
      ...
      <div className="flex flex-col gap-3 pt-5">
        <h1 className="text-2xl">
          Rent starting at{" "}
          <span className="text-[#FF311F] text-4xl font-bold">
            {/* @ts-ignore */}
        ...
        <button
          ...
          className="bottom-0 outline outline-[#FF311F] uppercase outline-4 rounded-full px-6 py-2 text-xl transition duration-50 ease-in-out bg-inherit hover:bg-[#FF311F] hover:text-white text-[#FF311F] hover:scale-110 hover:shadow-lg hover:shadow-[#FF311F]"
        >
          pay rent
        </button>
      </div>
